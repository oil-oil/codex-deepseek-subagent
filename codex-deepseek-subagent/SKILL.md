...
description: 仅在用户要求配置、选择或切换模型、检查、测试、修复、停用或卸载 Codex 的 DeepSeek 原生子 Agent 时使用；支持 DeepSeek V4 Flash Vision Experimental 与 V4 Pro。普通 DeepSeek API 问题和已配置后的日常编码任务不要触发。
---
...
- 支持 `deepseek-v4-flash-vision-exp` 与 `deepseek-v4-pro`。Vision Experimental 更快、更省，并支持文本、截图和图片；Pro 能力更强但只处理文本。两者默认思考程度均为 `high`。
- `deepseek-v4-pro` 只处理文本；`deepseek-v4-flash-vision-exp` 可以处理文本、截图和图片，但视频仍需父 Agent 先转成文字事实。
- 日常任务只能直接调用：
